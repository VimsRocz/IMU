...
%   array of structs) and saved in MATLAB/results/Result_<IMU>_<GNSS>_TRIAD.mat.
...
for k = 1:numel(imu_paths)
    ...
    res_file = fullfile(resultsDir, sprintf('%s_%s_TRIAD_task5_results.mat', ...
        imuName, gnssName));
    if ~isfile(res_file)
        error('Expected Task 5 results %s not found', res_file);
    end
    r = load(res_file);
    save(fullfile(resultsDir, sprintf('Result_%s_%s_TRIAD.mat', ...
         imuName, gnssName)), '-struct', 'r');

    sum_file = fullfile(resultsDir, 'IMU_GNSS_summary.txt');
    if isfile(sum_file)
