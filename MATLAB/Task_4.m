...
% Convert integrated trajectories to ECEF and body frames for each method
C_n2e = C_NED_to_ECEF;
gnss_acc_ecef = (C_n2e * gnss_acc_ned')';
gnss_pos_body = struct(); gnss_vel_body = struct(); gnss_acc_body = struct();
imu_pos_ecef = struct(); imu_vel_ecef = struct(); imu_acc_ecef = struct();
imu_pos_body = struct(); imu_vel_body = struct(); imu_acc_body = struct();
for i = 1:length(methods)
    m = methods{i};
    C_B_N = C_B_N_methods.(m);
    C_N_B = C_B_N';
    imu_pos_ecef.(m) = (C_n2e * pos_integ.(m)')' + ref_r0';
    imu_vel_ecef.(m) = (C_n2e * vel_integ.(m)')';
    imu_acc_ecef.(m) = (C_n2e * acc_integ.(m)')';
    imu_pos_body.(m) = (C_N_B * pos_integ.(m)')';
    imu_vel_body.(m) = (C_N_B * vel_integ.(m)')';
    imu_acc_body.(m) = (C_N_B * acc_integ.(m)')';
    gnss_pos_body.(m) = (C_N_B * gnss_pos_ned')';
    gnss_vel_body.(m) = (C_N_B * gnss_vel_ned')';
    gnss_acc_body.(m) = (C_N_B * gnss_acc_ned')';
end
gnss = struct('pos_ned', gnss_pos_ned, 'vel_ned', gnss_vel_ned, 'acc_ned', gnss_acc_ned, ...
              'pos_ecef', gnss_pos_ecef, 'vel_ecef', gnss_vel_ecef, 'acc_ecef', gnss_acc_ecef, ...
              'pos_body', gnss_pos_body, 'vel_body', gnss_vel_body, 'acc_body', gnss_acc_body);
imu = struct('pos_ned', pos_integ, 'vel_ned', vel_integ, 'acc_ned', acc_integ, ...
             'pos_ecef', imu_pos_ecef, 'vel_ecef', imu_vel_ecef, 'acc_ecef', imu_acc_ecef, ...
             'pos_body', imu_pos_body, 'vel_body', imu_vel_body, 'acc_body', imu_acc_body);
...
t_g = t_g_u; %#ok<NASGU>
save(t4_mat, 'rid','t_g','gnss_time','imu_pos_g','imu_vel_g','imu_acc_g', ...
    'gnss','imu','-v7');
...
result_struct = struct('gnss', gnss, 'imu', imu, ...
                'acc_biases', acc_biases, 'gyro_biases', gyro_biases, ...
                'scale_factors', scale_factors, 'truth_pos_ecef', truth_pos_ecef, ...
                'truth_vel_ecef', truth_vel_ecef, 'truth_time', truth_time, ...
                't_imu', t_imu, 't_gnss', t_gnss, 'gnss_time', gnss_time, ...
                'imu_time', t_imu, 'gnss_pos_ned_imuT', gnss_pos_ned_imuT, ...
