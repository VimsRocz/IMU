%% GNSS_IMU_Fusion_single.m - Standalone MATLAB version of GNSS_IMU_Fusion.py
% This script reproduces the Python pipeline in one file. It performs
% IMU/GNSS alignment, dead-reckoning and simple Kalman fusion.
% Results are written to the 'results' directory like the Python code.
...
omega_ie_ned = EARTH_RATE * [cosd(lat0); 0; sind(lat0)];

% Subtask 1.4: Validate magnitudes
assert(abs(norm(g_ned) - GRAVITY) < 1e-3, 'Gravity magnitude check failed');
assert(abs(norm(omega_ie_ned) - EARTH_RATE) < 1e-6, 'Earth rate magnitude check failed');
...
a_ned = (R_nb * f_b')' + repmat([0 0 GRAVITY],size(f_b,1),1);
...
function [lat_deg, lon_deg, alt] = ecef2geodetic(x, y, z)
    a = 6378137.0; e_sq = 6.69437999014e-3;
    p = sqrt(x.^2 + y.^2); b = a * sqrt(1 - e_sq);
    theta = atan2(z .* a, p .* b); ep_sq = (a^2 - b^2) / b^2;
    lon_rad = atan2(y, x);
    lat_rad = atan2(z + ep_sq .* b .* sin(theta).^3, ...
                    p - e_sq .* a .* cos(theta).^3);
    N = a ./ sqrt(1 - e_sq .* sin(lat_rad).^2);
    alt = p ./ cos(lat_rad) - N;
    lat_deg = rad2deg(lat_rad); lon_deg = rad2deg(lon_rad);
end
