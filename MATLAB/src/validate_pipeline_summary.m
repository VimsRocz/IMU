...
for mi = 1:numel(methods)
    ...
    for di = 1:numel(datasets)
        ds = datasets{di};
        fname = sprintf('%s/IMU_%s_GNSS_%s_%s_task5_results.mat', get_results_dir(), ds, ds, method);
        if ~isfile(fname)
...
for mi = 1:numel(methods)
    ...
    for di = 1:numel(datasets)
        ds = datasets{di};
        validate_3sigma( ...
            sprintf('%s/IMU_%s_GNSS_%s_%s_kf_output.mat', get_results_dir(), ds, ds, method), ...
            'STATE_X001.txt');
    end
end
