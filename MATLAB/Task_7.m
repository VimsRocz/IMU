...
try
    if exist('truth_data','var') && ~isempty(truth_data) && size(truth_data,2) >= 12
        ...
        if exist('slerp_series','file') || exist('attitude_tools','file')
            ...
            q_tru_slerp = attitude_tools('slerp_series', (t_truth - t_truth(1)).', q_tru_raw.', t_est.');
            q_tru_interp = q_tru_slerp.';  % size = length(t_est) × 4
        else
            % Fallback: linear interp each component + renormalize
            q_tru_interp = interp1(t_truth, q_tru_raw, t_est, 'linear', 'extrap');
...
try
    att_out_dir = fullfile(resultsDir, 'task7', runTag);
    if ~exist(att_out_dir, 'dir'); mkdir(att_out_dir); end
    est_file_path = fullfile(resultsDir, sprintf('%s_task5_results.mat', runTag));
    if isfile(est_file_path) && isfile(truth_txt)
        fprintf('[Task7] Running attitude quaternion comparison and overlays...\n');
        task7_fused_truth_error_analysis(est_file_path, truth_txt, att_out_dir);
    else
        warning('[Task7] Skipping attitude comparison; missing files: %s or %s', est_file_path, truth_txt);
    end
catch ME
