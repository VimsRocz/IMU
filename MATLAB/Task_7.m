...
for i=1:3
    ...
    nexttile; plot(t_est, pos_residual(:,i),'LineWidth',1.0); grid on; title(sprintf('Pos residual %s [m]', lbl{i})); xlabel('Time [s]');
end
png1 = fullfile(resultsDir, sprintf('%s_task7_pos_residual_NED.png', runTag));
exportgraphics(f1, png1, 'Resolution',150); try, savefig(f1, strrep(png1, '.png', '.fig')); catch, end; close(f1);
...
for i=1:3
    ...
    nexttile; plot(t_est, vel_residual(:,i),'LineWidth',1.0); grid on; title(sprintf('Vel residual %s [m/s]', lbl{i})); xlabel('Time [s]');
end
png2 = fullfile(resultsDir, sprintf('%s_task7_vel_residual_NED.png', runTag));
exportgraphics(f2, png2, 'Resolution',150); try, savefig(f2, strrep(png2, '.png', '.fig')); catch, end; close(f2);
...
function [P,V] = extractNED(T)
    P=[]; V=[]; cn = findCol(T, {'pos_n','ned_n','north'}); ce = findCol(T, {'pos_e','ned_e','east'}); cd = findCol(T, {'pos_d','ned_d','down'});
    if ~isempty(cn)&&~isempty(ce)&&~isempty(cd), P = [T.(cn), T.(ce), T.(cd)]; end
    vn = findCol(T, {'vel_n','ned_vn','vn','north_vel'}); ve = findCol(T, {'vel_e','ned_ve','ve','east_vel'}); vd = findCol(T, {'vel_d','ned_vd','vd','down_vel'});
    if ~isempty(vn)&&~isempty(ve)&&~isempty(vd), V = [T.(vn), T.(ve), T.(vd)]; end
end
