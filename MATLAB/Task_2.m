...
% ------------------------------------------------------------------
task1_file = fullfile('MATLAB/results', ['Task1_init_' tag '.mat']);
if ~isfile(task1_file)
    % Fallback to method-agnostic filename for backwards compatibility
    alt_file = fullfile('MATLAB/results', ['Task1_init_' pair_tag '.mat']);
    if isfile(alt_file)
        task1_file = alt_file;
    end
end
...
save(fullfile('MATLAB/results', ['Task2_body_' tag '.mat']), 'dt_imu', 'g_body', 'g_body_scaled', 'omega_ie_body', 'accel_bias', 'gyro_bias');
fprintf('Body-frame vectors and biases saved to %s\n', fullfile('MATLAB/results', ['Task2_body_' tag '.mat']));
