...
% Convert measured GNSS data to body frame for overlay
eul_gnss = interp1(time, euler_log', gnss_time, 'linear', 'extrap')';
pos_gnss_body = zeros(size(gnss_pos_ned));
vel_gnss_body = zeros(size(gnss_vel_ned));
acc_gnss_body = zeros(size(gnss_accel_ned));
for k = 1:length(gnss_time)
    C_BN = euler_to_rot(eul_gnss(:,k));
    pos_gnss_body(k,:) = (C_BN' * gnss_pos_ned(k,:)')';
    vel_gnss_body(k,:) = (C_BN' * gnss_vel_ned(k,:)')';
    acc_gnss_body(k,:) = (C_BN' * gnss_accel_ned(k,:)')';
end
...
plot_xyz_timeseries(time, pos_fused_body', vel_fused_body', acc_fused_body', ...
    gnss_time, pos_gnss_body', vel_gnss_body', acc_gnss_body', ...
