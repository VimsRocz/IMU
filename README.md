...
the MATLAB scripts. All intermediate files and figures generated by MATLAB are
now saved in `MATLAB/results/`. The helper `get_results_dir()` defined in the
`MATLAB` folder returns this path so scripts can locate the common results
