...
Passing `--show-measurements` adds the raw IMU and GNSS curves.  The resulting
 figures are written as ``results/<tag>_task6_overlay_state_<frame>.png`` and `.fig` files (both formats are saved).
...
* When running `run_all_methods.py`, plots are stored directly in
  `results/` as
  `<tag>_task7_3_residuals_position_velocity.png` and
  `<tag>_task7_4_attitude_angles_euler.png` (PNG/``.fig``). Here `<tag>` is the
  concatenation of the IMU dataset, GNSS dataset and method name, e.g.
  `IMU_X002_GNSS_X002_Davenport`, resulting in
  `IMU_X002_GNSS_X002_Davenport_task7_3_residuals_position_velocity.png`.
* The helper script `src/task7_plot_error_fused_vs_truth.py` produces
  `task7_fused_vs_truth_error.png` showing the fused minus truth velocity error.
...
* Subtask 7.5 generates `<tag>_task7_5_diff_truth_fused_over_time_<frame>.png` for
...
```
This creates one `all_datasets_<method>_comparison.png` per method in
...
writes `results/run_all_datasets/summary.csv`. Each row contains:
